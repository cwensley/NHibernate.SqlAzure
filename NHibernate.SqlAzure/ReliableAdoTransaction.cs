...
namespace NHibernate.SqlAzure
{
    ...
    public class ReliableAdoTransaction : AdoTransaction, ITransaction
    {
		...
		public static Task ExecuteWithRetryAsync(ReliableSqlDbConnection connection, System.Func<Task> action)
		{
			return connection.ReliableConnection.CommandRetryPolicy.ExecuteAsync(async () =>
			{
				if (connection.State != ConnectionState.Open)
